...
class Worker(object):
    ...
    def execute(self, sandbox: Environment, index):
        ...
        try:
            ...
            logger().info('Task %d, case %d Execute success', self.task.task_id, index)
    ...
    def running(self, sandbox: Environment):
        logger().info('Executing {id} @ {path}'.format(id=self.task.task_id, path=sandbox.path))

        total = self.cases.count

        for index in range(0, total):
            logger().info('Task %d, case %d, total %d', self.task.task_id, index, total)
